					...
					<div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 md:col-span-2">
						...
						{current && !dataUnavailable ? (
							<div className="grid grid-cols-2 md:grid-cols-4 gap-2">
								<div>
									<div className="text-sm  text-gray-500 dark:text-gray-400">Average Hashrate</div>
									<div className={`text-xl font-bold dark:text-white ${current.expectedHashrate && current.avgHashRate < current.expectedHashrate ? 'text-amber-500 dark:text-amber-400' : 'dark:text-white'}`}>
										~{(current.avgHashRate / 1000).toFixed(3)} <span className="text-sm text-gray-400">TH/s</span>
									</div>
									...
									<div className={`text-xs text-gray-400 dark:text-white text-right`}>
										Current: {(current.hashRate / 1000).toFixed(3)} TH/s
									</div>
								</div>
								<div>
								...
								<div>
									...
									{current.expectedHashrate && (
										<div className="text-xs text-gray-400 dark:text-gray-500">{((current.avgPower * 1000) / current.expectedHashrate).toFixed(1)} J/TH Expected</div>
									)}
								</div>
							</div>
						) : apiError ? (
