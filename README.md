...
**This software is capable of severely overclocking or underclocking your bitaxe hardware. This could lead to damaging your bitaxe device and/or your power supply. Use at your own risk**. The application cannot be held responsibly for any breakages and ultimately, if you are running this software, you are the responsible party if anything breaks.
...
Choose how hot you want to drive your voltage regulator and enter the **Max VR** value. The bitaxe UI does not offer an opinion on any maximum but personal findings are that 90 or above is possible with adequate cooling in place.

Note that overclocking you device will greatly increase its power requirements. The value reported in the UI is normally lower than the actual power draw so you need to be careful you are using an appropriate power supply.
...
Your chosen **Max Freq** value should be chosen to match your environment. If the device is in a temperature controlled environment this should be easy to find. In the real world, rooms get colds at night and warm during the day. **Bitaxe Stabiliser** will automatically throttle or speed up your device to compensate for fluctuations. As the room gets warmer (sunshine or heating on) then the frequency will step down.
...
Constant monitoring leads to the application having to run 24 hours a day, 7 days a week. This implies that you have to have a PC on permanently to run this application.
...
The simplest way to get **Bitaxe Stabiliser** running is to use docker. Follow the instructions on the docker web site to install docker on your system.

You have 2 methods of running the application:

- Clone the Bitaxe Stabiliser repository from github and build your own docker image
- Use the prebuilt image on docker hub and create a 'docker-compose.yml' file.
...
services:
  # Config for bitaxe device 1
  bitaxe-stabiliser-g1:
    <<: *base
    environment:
      - BITAXE_IP=192.168.0.101
    ports:
      - "3001:3000"

  # Config for bitaxe device 2
  bitaxe-stabiliser-g2:
    <<: *base
    environment:
      - BITAXE_IP=192.168.0.102
    ports:
      - "3002:3000"
```

You can add as many services as you have bitaxe devices to control. Each service will need a unique name (e.g. bitaxe-stabiliser-g1), unique IP address and unique port to access the UI on. Cut and paste a block for each of them.
...
You can select any port to run the server on. By default it is port 3001 but you can change this by editing the ports line to be **- "1234:3000"** to run on localhost:1234. When the docker service is started, navigate to <http://localhost:3001> and you will see the **Bitaxe Stabiliser** UI connected to the bitaxe device with the specified IP.

The deviceData volume is required to persist your settings between restarts. By default, all settings and data will be stored in a **deviceData** folder where the **docker-compose.yml** files is located. In **deviceData**, you should find a subfolder for each bitaxe you point the application to monitor. Create this folder if it does not exist.

Rename the second service to bitaxe-stabiliser-2 and select a different port for the second UI instance.
You can host as many instances of **Bitaxe Stabiliser** as your computer can handle. I would expect an ancient PC could support dozens of instances. Be aware that constantly running a PC as a server may use more power than a single bitaxe!

If you want to build your own docker image, then follow these instructions.
...
There are other environment variables that can be set in your `docker-compose.yml`. There is no need to pass these as they should default to match your current bitaxe settings and any changes you make will persist between restarts (because of the persistent deviceData docker volume).
...
If you are so inclined, you can manually build a docker image for yourself without using `docker-compose.yml`. Consult the docker documentation if you want to know more about running from the command line.
...
The application stores data in a folder `./deviceData` on the host. A folder named after the bitaxe hostname (or IP address if the hostname cannot be determined from the bitaxe's API) will include several files for settings and history data.
...
Note that being over optimistic may cause your bitaxe to overheat (quickly). Please use caution and be conservative until you discover what you particular bitaxe / power supply is capable of. Bitaxes are very good at detecting overheating before damaging the board but it cannot control your power supply. Too high a frequency / core voltage and your power supply may pop. **YOU SHOULD NOT BE RUNNING A BITAXE AT 36W on a 40W POWER SUPPLY**. Look up the 80% rule if you are unsure about what this is.
...
This software is provided by myself to the open source community and is something that I have built for my own use in my own free time. My aim for this software was to run my bitaxes hot to provide ambient supplementary heating for my office on colder days. Higher hash rates are also good in my opinion :)

If you find yourself using this software, like it and feel like giving something back then I have left a bitcoin address in the UI that you are welcome to send any donations as thanks. Anything will be much appreciated and unexpected. I hope you like this application. If you don't then that is also fine but don't moan to me about it!
